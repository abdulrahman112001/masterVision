...
import { AllChartsAPI_TP } from "./Types&Validation";
import AddCharType from "./AddCharType";

function MainChartType() {
  ...
  const endpoint = `accounting/charttype?${searchParams.toString()}`;
  const { data, refetch, isSuccess, isFetching, isLoading } =
    useFetch<AllChartsAPI_TP>({
      endpoint: endpoint,
      queryKey: [endpoint],
      onSuccess: () => {
  ...
  return (
    <div>
      ...
      >
        <AddCharType refetch={refetch} update={MainData} data={data?.data?.data} />
      </ModalTemplate>
