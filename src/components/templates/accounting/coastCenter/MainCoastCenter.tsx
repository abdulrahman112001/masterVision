...
import { AllCoastsAPI_TP } from "./Types&Validation";
import { generateColumns } from "./generateColumns";
import AddCoast from "./AddCoast";


function MainCoastCenter() {
  ...
  const endpoint = `accounting/costcenter?${searchParams.toString()}`;
  const { data, refetch, isSuccess, isFetching, isLoading } =
    useFetch<AllCoastsAPI_TP>({
      endpoint: endpoint,
      queryKey: [endpoint],
      onSuccess: () => {
  ...
  return (
    <div>
      ...
      >
        <AddCoast refetch={refetch} update={MainData} data={data?.data?.data} />
      </ModalTemplate>
...
export default MainCoastCenter;
