...
import { AllAdminsAPI_TP } from "./Types&Validation";
...
function MainAdmins() {
  ...
  const endpoint = `master-data/users?${searchParams.toString()}`;
  const { data, refetch, isSuccess, isFetching, isLoading } =
    useFetch<AllAdminsAPI_TP>({
      endpoint: endpoint,
      queryKey: [endpoint],
      onSuccess: () => {
  ...
  return (
    <div>
      ...
      >
        <AddAdmin refetch={refetch} update={MainData} data={data?.data?.data} />
      </ModalTemplate>
